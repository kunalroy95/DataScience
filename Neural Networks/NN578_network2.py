...
class LogLikelihood(object):
    ...
    def derivative(a, y):
        ...
        if a[target_index] == 1:
            ...
            vector[target_index] = -1/(a[target_index])
            
        else:
            
            vector[target_index] = 0
...
class Tanh(object):
    ...
    def fn(z):
        ...
        return (z_plus - z_minus) / (z_plus + z_minus)
...
class Network(object):
    ...
    def backprop(self, x, y):
        ...
        for b, w in zip(self.biases, self.weights):
            ...
            activation_new = self.dropout(activation, 0.5)
            
            
            activations.append(activation_new)
        ...
        for l in range(2, self.num_layers):
            ...
            delta_new = self.dropout(delta, 0.5)
            
            nabla_b[-l] = delta_new
            nabla_w[-l] = np.dot(delta_new, activations[-l - 1].transpose())
    ...
    def total_cost(self, data, lmbda, convert=False):
        ...
        if self.regularization == 'L2':
            ...
            cost += (lmbda/len(data)) * sum(np.abs(w).sum() for w in self.weights)
    ...
    def dropout(self, X, drop_prob):
        ...
        mask = np.random.binomial(0, 1.0, X.shape) < keep_prob
